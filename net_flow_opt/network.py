...
class Plan:
    ...
    def _dates(self, grouping_structure):
        ...
        G_dates = np.zeros(len(G))
        for i, g in enumerate(G):
            if len(g) == 1:
                G_dates[i] = self.system.components[g[0]].x_star
            else:
                A = [self.activities[j] for j in g]
                # calculate group date
                f = lambda x: sum([a.h(x - a.t) for a in A])
                sol = minimize(
                    fun=f,
                    x0=np.mean([a.t for a in A])
                )
                G_dates[i] = sol.x

        idx = np.argsort(G_dates)
        G = G[idx]
        G_dates = G_dates[idx]

        # add constraints to list
        A = np.zeros((len(G) - 1, len(G)))
        for i in range(len(G) - 1):
            A[i, i] = 1
            A[i, i + 1] = -1
        ub = np.zeros(len(G) - 1)
        for i in range(len(G) - 1):
            # pick the duration of the longest activiti in the group
            ub[i] = - max([a.d for a in [self.activities[j] for j in G[i + 1]]])

        def f(x):
            return sum(
                [
                    sum(
                        a.h(t - a.t) for a in [self.activities[j] for j in G[i]]
        ...
        sol = minimize(
            fun=f,
            x0=G_dates,
            constraints=LinearConstraint(A=A, ub=ub)
        )
        t = np.zeros(self.system.N)
        for i, g in enumerate(G):
            for j in g:
                t[j] = sol.x[i]
